...
3. For Windows "Failed to open a session for the virtual machine FourGuysBank". When the error details show "AMD-V is disabled in the BIOS (or by the host OS) (VERR_SVM_DISABLED), go to BIOS during boot: OC (overclock settings) -> CPU features -> SVM mode (change to enabled). If the error still shows up after, restart the computer again.
